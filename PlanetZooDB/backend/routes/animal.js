...
router.get('/get',auth.authenticateToken,(req,res,next) =>{
    var query = "select a.id,a.name,a.description,a.status,d.id as dlcId,d.name as dlcName from animal as a INNER JOIN dlc as d where a.dlcId = d.id";
...
router.patch('/update',auth.authenticateToken,checkRole.checkRole,(req,res,next) =>{
    ...
    connection.query(query,[animal.name,animal.dlcId,animal.description,animal.id],(err,results) =>{
        if(!err){
...
router.patch('/updateStatus',auth.authenticateToken,checkRole.checkRole,(req,res,next) =>{
    ...
    connection.query(query,[user.status,user.id],(err,results) =>{
        if(!err){
            ...
            return res.status(200).json({message:"Animal status updated successfully"});
        }
        else{
            return res.status(500).json(err);
        }
